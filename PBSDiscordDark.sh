...
function checkSupported {   
    if [ "$OFFLINE" = false ]; then
        local SUPPORTED=$(curl -f -s "$BASE_URL/meta/PBSsupported")
    else
        local SUPPORTED=$(cat "$OFFLINEDIR/meta/PBSsupported")
    fi

    if [ -z "$SUPPORTED" ]; then 
        if [ "$OFFLINE" = false ]; then
            echo -e "${WARN}Could not reach supported version file ($BASE_URL/meta/PBSsupported). Skipping support check.${REG}"
        else
            echo -e "${WARN}Could not find supported version file ($OFFLINEDIR/meta/PBSsupported). Skipping support check.${REG}"
        fi
    else 
